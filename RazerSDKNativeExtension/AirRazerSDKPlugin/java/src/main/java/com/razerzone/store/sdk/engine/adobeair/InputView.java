...
public class InputView extends View implements View.OnClickListener {

	private static final String TAG = InputView.class.getSimpleName();
	
	private static boolean sEnableLogging = false;
	
	private static InputView sInstance = null;
    ...
    public InputView(Context context, AttributeSet attrs) {
    	super(context, attrs);
		if (sEnableLogging) {
			Log.d(TAG, "InputView(Context context, AttributeSet attrs)");
		}
        init();
    }
    ...
    public InputView(Context context, AttributeSet attrs, int defStyle) {
		...
		if (sEnableLogging) {
			Log.d(TAG, "InputView(Context context, AttributeSet attrs, int defStyle)");
		}
        init();
    }
    ...
    public InputView(Context context) {
        super(context);
		if (sEnableLogging) {
			Log.d(TAG, "InputView(Context context)");
		}
        init();
    }
	...
	public boolean onGenericMotionEvent(MotionEvent motionEvent) {
		...
		*/
    	...
    	processAxis(stateAxises, playerNum, Controller.AXIS_LS_X, motionEvent.getAxisValue(Controller.AXIS_LS_X));
    	processAxis(stateAxises, playerNum, Controller.AXIS_LS_Y, motionEvent.getAxisValue(Controller.AXIS_LS_Y));
    	processAxis(stateAxises, playerNum, Controller.AXIS_RS_X, motionEvent.getAxisValue(Controller.AXIS_RS_X));
    	processAxis(stateAxises, playerNum, Controller.AXIS_RS_Y, motionEvent.getAxisValue(Controller.AXIS_RS_Y));
    	processAxis(stateAxises, playerNum, Controller.AXIS_L2, motionEvent.getAxisValue(Controller.AXIS_L2));
    	processAxis(stateAxises, playerNum, Controller.AXIS_R2, motionEvent.getAxisValue(Controller.AXIS_R2));
	...
	private void processAxis(SparseArray<Float> stateAxises, final int playerNum, final int axis, final float val) {
		...
		if (null != activity) {
			Runnable runnable = new Runnable() {
				...
				public void run() {
					try {
						if (null != Plugin.getFREContext()) {
							JSONObject jsonObject = new JSONObject();
							jsonObject.put("playerNum", playerNum);
							jsonObject.put("axis", axis);
							jsonObject.put("value", val);
                            Plugin.getFREContext().dispatchStatusEventAsync("Axis", jsonObject.toString());
						} else {
						...
						public void run() {
							try {
								if (null != Plugin.getFREContext()) {
                                    ...
                                    Plugin.getFREContext().dispatchStatusEventAsync("ButtonUp", jsonObject.toString());
								} else {
									// activity has been destroyed
									activity.finish();
								}
						...
						public void run() {
							try {
								if (null != Plugin.getFREContext()) {
									...
									jsonObject.put("playerNum", playerNum);
									jsonObject.put("button", keyCode);
                                    Plugin.getFREContext().dispatchStatusEventAsync("ButtonDown", jsonObject.toString());
								} else {
									// activity has been destroyed
									activity.finish();
								}
	...
	public static void clearButtonStatesPressedReleased() {
		...
		if (sStateButtonDown.size() < Controller.MAX_CONTROLLERS) {
			return;
		}
		
		if (sStateButtonUp.size() < Controller.MAX_CONTROLLERS) {
			return;
		}
		
		if (sLastStateButtonDown.size() < Controller.MAX_CONTROLLERS) {
			return;
		}
		
		if (sLastStateButtonUp.size() < Controller.MAX_CONTROLLERS) {
			return;
		}
